  ...
  global: {
    stubs: {
      ...
      ConfirmDialog: {
        ...
        template: '<div v-if="show"><span>{{ message }}</span><button data-test="confirm-overwrite" @click="$emit(\'confirm\')">confirm</button><button data-test="cancel-overwrite" @click="$emit(\'cancel\')">cancel</button></div>'
      }
    }
  }
})
...
describe('BulkEditUserModal', () => {
  ...
  it('submits only the enabled RPM field and preserves zero as unlimited', async () => {
    ...
    expect(wrapper.text()).toContain('admin.users.bulkLimits.rpmUnlimitedValue')
  ...
  it('does not call the API when overwrite confirmation is cancelled', async () => {
    ...
    await wrapper.get('[data-test="concurrency-input"]').setValue('9')
    await wrapper.get('form').trigger('submit')
    await wrapper.get('[data-test="cancel-overwrite"]').trigger('click')
    await flushPromises()
