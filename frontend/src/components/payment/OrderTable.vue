<template>
  <DataTable :columns="columns" :data="orders" :loading="loading">
    ...
    <template #cell-pay_amount="{ value, row }">
      <div class="text-sm">
        <span class="font-medium text-gray-900 dark:text-white">{{ formatOrderAmount(value, row) }}</span>
...
function formatOrderAmount(value: number, order: PaymentOrder): string {
  return props.showUser ? currencySymbol(order.currency) + value.toFixed(2) : formatCurrency(value)
}

function formatCreditedAmount(value: number): string {
  return props.showUser ? currencySymbol('USD') + value.toFixed(2) : formatCurrency(value)
}
